...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for (ap=1;ap<=I/2;ap++) {
        ...
        for (a=ap;a<=I/2;a++) {
            ...
            t = P_ij0 + P_ij1;
            g->elt[a-1][ap-1] = (P_ij0 * log2b * log(2.0 * P_ij0 / t) + P_ij1 * log2b * log(2.0 * P_ij1 / t)) + 1E-15;
        }
    }
    ...
    for (a=1;a<=1+I/2-K/2;a++) {
        S->elt[0][a-1] = g->elt[a-1][0];
    }
    
    for (k=2;k<=K/2;k++) {
        for (a=k;a<=k+I/2-K/2;a++) {
