...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    MATRIX *P;    /*input 0*/
    MATRIX *Kin;  /*input 1*/
    MATRIX *S = matrix("S",0,0);            /* Matrix of States*/
    MATRIX *h = matrix("h",0,0);            /* Matrix of local decisions */
    MATRIX *astar = matrix("astar",0,0);    /* Matrix of quantizer boundaries */
    MATRIX *Qstar = matrix("Qstar",0,0);    /* Quantizer Matrix */
    ...
    for (ap=1;ap<=I;ap++) {
        ...
        for (a=ap;a<=I;a++) {
            P_i = P_i + P->elt[a-1][0];
            g->elt[a-1][ap-1] =  -P_i * log2b * log(P_i) + 1E-15;
        }
    }
