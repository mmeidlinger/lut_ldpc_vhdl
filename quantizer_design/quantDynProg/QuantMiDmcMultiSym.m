function [Qout,mi] = QuantMiDmcMultiSym(Pin,K)
% [Q,MI] = QuantMiDmcMulti(P,K)
%
...
for kk = K/2:-1:1
    ...
    for ii = 1:size(Q,1)
        ...
        if length(t) == 1
            ...
            Q(ii,K/2-kk+2) = t(1);
            Qt = repmat(Q(ii,1:K/2-kk+1),length(t)-1,1);
            tp = t(:);
            Qt = [Qt tp(2:end)];
            Qnew = [Qnew; Qt];
        end
...
for ii = 1:size(Qlist,1)
    Q{ii} = zeros(K,I);
    for kk = 1:K/2
        Q{ii}(K/2-kk+1, Qlist(ii,kk+1)+1:Qlist(ii,kk)) = 1;
    end
    Q{ii}(:, I/2+1:end) = Q{ii}(end:-1:1,I/2:-1:1);
end
